...
// node_modules/frame-master-plugin-apply-react/src/client-shell.tsx
function ClientShell({ children }) {
  return children;
}
// node_modules/frame-master/src/utils.ts
function join(...parts) {
...
// node_modules/frame-master-plugin-apply-react/src/layout.tsx
var jsx_dev_runtime = __toESM(require_jsx_dev_runtime(), 1);
function getRelatedLayoutFromPathname(pathname, routes) {
  const paths = pathname ? pathname.split("/").filter(Boolean) : [];
  const layouts = Object.assign({}, ...Object.entries(routes).filter(([pathname2, layout]) => pathname2.endsWith("layout")).map(([k, v]) => ({ [k]: v })));
  const relatedLayouts = [];
  if (layouts["/layout"])
    relatedLayouts.push(layouts["/layout"]);
  if (paths.length === 0)
  ...
  for (const path of paths) {
    ...
    if (typeof layouts[layoutPathToTest] == "undefined")
      continue;
...
// node_modules/frame-master-plugin-apply-react/src/utils.ts
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (rootElement) {
    ...
    const PageToRender = client_routes_default[pathname];
    if (!PageToRender) {
      console.error("No page found for pathname:", window.location.pathname);
      console.error("Available routes:", client_routes_default);
      throw new Error("pathname does not exists");
    }
    const WrappedPage = getRelatedLayoutFromPathname(pathname, client_routes_default).reverse().reduce((Prev, Curr) => /* @__PURE__ */ jsx_dev_runtime2.jsxDEV(Curr, {
      children: Prev
    }, undefined, false, undefined, this), /* @__PURE__ */ jsx_dev_runtime2.jsxDEV(PageToRender, {}, undefined, false, undefined, this));
    import_client.hydrateRoot(rootElement, /* @__PURE__ */ jsx_dev_runtime2.jsxDEV(import_react.StrictMode, {
      children: /* @__PURE__ */ jsx_dev_runtime2.jsxDEV(ClientShell, {
